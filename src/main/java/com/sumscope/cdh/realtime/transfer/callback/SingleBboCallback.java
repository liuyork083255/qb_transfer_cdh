 ...
 */
@Component
public class SingleBboCallback extends AbstractCallback {

    private static final Logger LOGGER = LoggerFactory.getLogger(SingleBboCallback.class);
    ...
    public boolean processString(String message) {
        ...
        try {
            ...
            LOGGER.error(String.format("sourceDisruptor publish bbo_1 event fail,exception=>[%s].  message=>[%s]",e.getMessage(),message));
        }
        return true;
    }
